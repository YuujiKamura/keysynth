...
impl NsfEmu {
    fn from_data(data: &[u8], sample_rate: u32) -> Result<Self, String> {
        ...
        eprintln!("render_nsf: gme_open_data path failed; forcing NSF type via gme_nsf_type");
...
fn run() -> Result<(), String> {
    ...
    let mut writer =
        hound::WavWriter::create(&args.output, spec).map_err(|e| format!("wav create: {e}"))?;
    for s in &buf {
        writer
            .write_sample(*s)
