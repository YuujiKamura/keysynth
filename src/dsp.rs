...
mod tests {
    ...
    fn eq3_flat_is_passthrough() {
        ...
        for (a, b) in buf.iter().zip(original.iter()) {
            assert!((a - b).abs() < 1e-5, "flat EQ should pass through, got {a} vs {b}");
        }
    }
    ...
    fn compressor_reduces_loud_peaks() {
        ...
        assert!(peak > 0.05, "compressor should not silence the signal, got {peak}");
    }
    ...
    fn process_track_chain_runs_clean_with_flat_settings() {
        ...
        for (a, b) in buf.iter().zip(original.iter()) {
            assert!((a - b).abs() < 1e-4, "expected near-passthrough, got {a} vs {b}");
        }
    }
